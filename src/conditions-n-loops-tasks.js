...
function isContainNumber(num, digit) {
  let rest = num;
  while (rest) {
    const newRest = Math.trunc(rest / 10);
    const item = rest - newRest * 10;
    if (item === digit) {
      return true;
    }
    rest = newRest;
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < halfSize; i += 1) {
    for (let j = shift; j < size - 1 - shift; j += 1) {
      [
        res[i][j],
        res[j][size - 1 - i],
        res[size - 1 - i][size - 1 - j],
        res[size - 1 - j][i],
      ] = [
        res[size - 1 - j][i],
        res[i][j],
